...
- R7: The oracle's vote for a new oracle posting price. (Can be empty if not voting to change price)
...
###### Oracle Post Price
Price to payout oracles which is held in R7 of the [Live Epoch](<#Stage-Live-Epoch>) input box.
...
The amount of Ergs is based off of the `Oracle Post Price` which is found in R7 of Input #1.
...
10. A payment box output is generated for every single oracle who's address is in the list in Output #1 R7 which has a total amount of Ergs inside equal to the `Oracle Post Price`.
10. Output #2 is generated for the collector who's address is in R6 of Output #1, which has a total Ergs as dictated by the `Collector Payout Function`.
11. Each payment box has a total amount of Ergs inside equal to the value held in R7 of the Input #1.
12. Each data-input [Datapoint](<#Stage-Datapoint>) box has an R5 that is equal to Input #1 box id.
13. If 75%+ of all oracle [Datapoint](<#Stage-Datapoint>) boxes have the same value in R7, then said value is placed in R9 of Output #1. Else the R7 from Input #1 is used.
14. At least 1 valid data-input box is provided.
15. Output #1 address is equal to the address held in R6 of Input #1.
16. Every data-input [Datapoint](<#Stage-Datapoint>) box has a datapoint within the margin of error.
---
